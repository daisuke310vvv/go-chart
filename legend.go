...
// Legend returns a legend renderable function.
func LegendStackedBarChart(c *StackedBarChart, userDefaults ...Style) Renderable {
	return func(r Renderer, cb Box, chartDefaults Style) {
		legendDefaults := Style{
			FillColor:   drawing.ColorWhite,
			FontColor:   DefaultTextColor,
		...
		var labels []string
		var lines []Style
		for index, s := range c.Bars {
			labels = append(labels, s.Name)
			lines = append(lines, c.styleDefaultsStackedBarValue(index))
			// if !s.GetStyle().Hidden {
			// 	if _, isAnnotationSeries := s.(AnnotationSeries); !isAnnotationSeries {
			// 		labels = append(labels, s.GetName())
			// 		lines = append(lines, s.GetStyle().InheritFrom(c.styleDefaultsSeries(index)))
